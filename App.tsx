...
const App: React.FC = () => {
  ...
  useEffect(() => {
    const init = async () => {
      try {
        ...
        const mainAdmins = [
          { id: '1', email: 'credplusemp@gmail.com', password: import.meta.env.VITE_ADMIN_PASSWORD || '', role: UserRole.ADMIN },
          { id: '2', email: 'lellisflavio@gmail.com', password: import.meta.env.VITE_ADMIN_PASSWORD || '', role: UserRole.ADMIN }
        ];
    ...
    switch (activeTab) {
      ...
      case 'admin':
        ...
        if (!perms.isAdmin) return (
          <div className="p-10 text-center font-black uppercase text-red-500">Acesso Negado</div>
        );
        const pendingClients = db.clients.filter((c: any) =>
          c.approvalStatus === ClientApprovalStatus.PRE_CADASTRO ||
          c.approvalStatus === ClientApprovalStatus.AGUARDANDO_ADM
        );
        return (
          <div className="p-4 md:p-8">
            <h2 className="text-2xl font-black uppercase tracking-tighter mb-6 text-slate-800">
              Aprovar Cadastros
              {pendingClients.length > 0 && (
                <span className="ml-3 bg-amber-400 text-white text-sm px-3 py-1 rounded-full font-bold">
            ...
            {pendingClients.length === 0 ? (
              ...
              <div className="space-y-4">
                {pendingClients.map((client: any) => {
                  const createdByUser = db.users.find((u: any) => u.id === client.createdBy);
                  return (
                    <div key={client.id} className="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm">
  ...
  if (!user) return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-emerald-900 via-emerald-800 to-slate-900 p-6">
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          <Logo size="xl" className="mx-auto mb-4" />
          <h1 className="text-3xl font-black text-white tracking-tight">CredPlus</h1>
          <p className="text-emerald-300 text-sm font-medium mt-1 uppercase tracking-widest">Gestão Financeira</p>
        </div>
        <div className="bg-white rounded-3xl shadow-2xl p-8">
          <p className="text-slate-500 text-sm text-center mb-6 font-medium">Acesse sua conta para continuar</p>
          <form onSubmit={e => {
            e.preventDefault();
            ...
            <div>
              ...
              <input type="email" required className="w-full p-4 bg-slate-50 border border-slate-200 rounded-2xl text-slate-800 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent transition-all" placeholder="seu@email.com" value={authForm.email} onChange={e => setAuthForm({ ...authForm, email: e.target.value })} />
            </div>
            <div>
              <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1.5">Senha</label>
              <input type="password" required className="w-full p-4 bg-slate-50 border border-slate-200 rounded-2xl text-slate-800 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent transition-all" placeholder="••••••••" value={authForm.password} onChange={e => setAuthForm({ ...authForm, password: e.target.value })} />
            </div>
            <button type="submit" className="w-full bg-emerald-600 hover:bg-emerald-700 active:bg-emerald-800 text-white py-4 rounded-2xl font-black text-sm uppercase tracking-wider flex items-center justify-center gap-2 transition-all shadow-lg shadow-emerald-200 mt-2">
              Entrar <ChevronRight size={18} />
            </button>
          </form>
        </div>
        <p className="text-center text-emerald-400/60 text-xs mt-6 font-medium">© 2025 CredPlus · Todos os direitos reservados</p>
      </div>
    </div>
  );
