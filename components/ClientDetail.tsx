        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-md">
           <div className="bg-white p-10 rounded-[2.5rem] w-full max-w-md shadow-2xl animate-in zoom-in duration-300">
              ...
              <form onSubmit={handleAdjSubmit} className="space-y-6">
                ...
                <div><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Valor</label><input type="number" step="0.01" required className="w-full p-4 bg-slate-50 rounded-2xl border font-bold mt-1" value={isNaN(adjData.amount) ? '' : adjData.amount} onChange={e=>setAdjData({...adjData, amount: parseFloat(e.target.value)})} /></div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-md">
           <div className="bg-white p-10 rounded-[2.5rem] w-full max-w-md shadow-2xl animate-in zoom-in duration-300">
              ...
              <form onSubmit={handleEditTrxSubmit} className="space-y-6">
                ...
                <div><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Valor</label><input type="number" step="0.01" required className="w-full p-4 bg-slate-50 rounded-2xl border font-bold mt-1" value={isNaN(editTrxData.amount) ? '' : editTrxData.amount} onChange={e=>setEditTrxData({...editTrxData, amount: parseFloat(e.target.value)})} /></div>
           ...
           <div className="bg-white p-10 rounded-[2.5rem] w-full max-w-md shadow-2xl animate-in zoom-in duration-300">
              ...
              <form onSubmit={handleEditCompSubmit} className="space-y-6">
                <div className="grid grid-cols-2 gap-4">
                  <div><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Mês (0-11)</label><input type="number" min="0" max="11" required className="w-full p-4 bg-slate-50 rounded-2xl border font-bold mt-1" value={isNaN(parseFloat(editCompData.month)) ? '' : editCompData.month} onChange={e=>setEditCompData({...editCompData, month: e.target.value})} /></div>
                  <div><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Ano</label><input type="number" required className="w-full p-4 bg-slate-50 rounded-2xl border font-bold mt-1" value={isNaN(parseFloat(editCompData.year)) ? '' : editCompData.year} onChange={e=>setEditCompData({...editCompData, year: e.target.value})} /></div>
                </div>
                ...
                <div><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Valor Juros</label><input type="number" step="0.01" required className="w-full p-4 bg-slate-50 rounded-2xl border font-bold mt-1" value={isNaN(parseFloat(editCompData.originalValue)) ? '' : editCompData.originalValue} onChange={e=>setEditCompData({...editCompData, originalValue: e.target.value})} /></div>
                <div><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Valor Recebido</label><input type="number" step="0.01" required className="w-full p-4 bg-slate-50 rounded-2xl border font-bold mt-1" value={isNaN(parseFloat(editCompData.paidAmount)) ? '' : editCompData.paidAmount} onChange={e=>setEditCompData({...editCompData, paidAmount: e.target.value})} /></div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-2 md:p-4 bg-emerald-900/40 backdrop-blur-md overflow-y-auto">
          <div className="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-lg my-auto animate-in fade-in zoom-in duration-300 border-b-8 border-slate-300 flex flex-col max-h-[95vh]">
            ...
            <form onSubmit={(e) => { e.preventDefault(); onRequestPayment(formData.interest, formData.amortization, formData.discount, formData.observation, new Date(formData.date).getTime() + 12 * 60 * 60 * 1000); setShowRequestModal(false); }} className="p-10 space-y-6 overflow-y-auto">
              ...
              <div className="space-y-1"><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Valor Juros</label><input type="number" step="0.01" className="w-full p-4 bg-slate-50 border rounded-2xl font-black" value={isNaN(formData.interest) ? '' : formData.interest} onChange={e=>setFormData({...formData, interest: parseFloat(e.target.value)})} /></div>
              <div className="space-y-1"><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Abatimento/Desconto de Juros</label><input type="number" step="0.01" className="w-full p-4 bg-slate-50 border rounded-2xl font-black text-amber-600" value={isNaN(formData.discount) ? '' : formData.discount} onChange={e=>setFormData({...formData, discount: parseFloat(e.target.value)})} /></div>
              <div className="space-y-1"><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Amortização de Capital</label><input type="number" step="0.01" className="w-full p-4 bg-slate-50 border rounded-2xl font-black text-emerald-600" value={isNaN(formData.amortization) ? '' : formData.amortization} onChange={e=>setFormData({...formData, amortization: parseFloat(e.target.value)})} /></div>
